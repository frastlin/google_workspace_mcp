...
def test_add_header_with_non_ascii_filename():
    ...
    assert (
        "filename*=" in content_disposition or "Prüfbericht" in content_disposition
    )  # Either encoded or raw UTF-8
...
def test_add_header_with_multiple_non_ascii_chars():
    ...
    assert (
        "filename*=" in content_disposition or "Kostenübersicht" in content_disposition
    )
