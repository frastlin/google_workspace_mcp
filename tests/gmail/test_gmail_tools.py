...
def _make_attachment_b64(content: bytes = b"hello world") -> str:
    """Helper to create a standard base64 string from bytes."""
...
# ============================================================
# Note: upstream uses 'content' key (standard base64) for attachments
...
class TestPrepareGmailMessage:
    ...
    def test_prepare_message_with_one_attachment(self):
        ...
        att = [
            {
                "filename": "test.txt",
                "content": _make_attachment_b64(content),
        ...
        atts = [
            {
                ...
                "content": _make_attachment_b64(f"content{i}".encode()),
                "mime_type": "text/plain",
            }
            for i in range(3)
        ]
    ...
    def test_prepare_message_binary_attachment(self):
        """Non-UTF-8 bytes round-trip correctly via base64 content."""
        binary_content = bytes(range(256))
        att = [
            {
                "filename": "binary.bin",
                "content": _make_attachment_b64(binary_content),
...
class TestSendGmailMessageImpl:
    ...
    async def test_send_with_inline_attachment(self):
        ...
        att = [
            {
                "filename": "test.txt",
                "content": _make_attachment_b64(b"file data"),
                "mime_type": "text/plain",
            }
        ]
        result = await _send_gmail_message_impl(
    ...
    @pytest.mark.asyncio
    async def test_send_with_from_name(self):
        """from_name is passed through to _prepare_gmail_message."""
        service = _make_mock_service()
        service.users().messages().send().execute.return_value = {"id": "sent_name"}
    ...
    async def test_send_reply_with_attachments(self):
        ...
        att = [
            {
                "filename": "data.csv",
                "content": _make_attachment_b64(b"a,b,c\n1,2,3"),
                "mime_type": "text/csv",
            }
        ]
        result = await _send_gmail_message_impl(
...
class TestDraftGmailMessageImpl:
    ...
    async def test_draft_with_attachment(self):
        ...
        att = [
            {
                "filename": "doc.txt",
                "content": _make_attachment_b64(b"draft attachment"),
                "mime_type": "text/plain",
            }
        ]
    ...
    async def test_draft_reply_with_attachment(self):
        ...
        att = [
            {
                "filename": "reply.pdf",
                "content": _make_attachment_b64(b"pdf bytes"),
                "mime_type": "application/pdf",
            }
        ]
        result = await _draft_gmail_message_impl(
