    ...
    for i, (expected_filename, _) in enumerate(test_attachments):
        ...
        if all(ord(c) < 128 for c in expected_filename):
            ...
            assert (
                ...
                or expected_filename in content_disposition
            )
...
def test_rfc_2231_encoding_format():
    ...
    assert (
        "filename*=" in content_disposition  # RFC 2231 encoded
        or "Prüfbericht.pdf" in content_disposition  # Raw UTF-8 (also valid)
    )
